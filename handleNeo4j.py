...
class App:
    ...
    def create_retweet(self, userId1, userId2, hashtag):
        with self.driver.session() as session:
            ...
            # Write transactions allow the driver to handle retries and transient errors
                ...
                result = session.write_transaction(
                    self._create_and_return_tweet, userId1, hashtag)
    ...
    @staticmethod
    def _create_and_return_tweet(tx, userId1, hashtag):
        query = (
...
def getFrnNetwork(user):
    ...
    app.close()
def getRetweetNetwork(userId,hashtag,retweetedFromId):
    app=connectToNeo4j()
