...
class App:
    ...
    def _create_and_return_friendship(tx, userId1, userId2):
        query=(
            "MERGE (p1:Person {id: $userId1}) ON MATCH SET p1.lastLoggedInAt = timestamp() ON CREATE SET p1.createdAt = timestamp()"
            "MERGE (p2:Person {id: $userId2}) ON MATCH SET p2.lastLoggedInAt = timestamp() ON CREATE SET p2.createdAt = timestamp()"
        ...
        try:
            return [{"p1": record["p1"]["id"], "p2": record["p2"]["id"]}
                    for record in result]
